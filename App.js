let i = 0;
async function addIntially(l) {
  const res = await axios.get(`https://api.tvmaze.com/shows`);
  const response = res.data;
  const length = response.length;
  for (let i = l; i < l + 20 && i < 100; i++) {
...
form.addEventListener("submit", async (e) => {
  ...
  const res = await axios.get(
    ` https://api.tvmaze.com/search/shows?q=${form.elements.query.value}`
  );
...
function addResult(shows, genre) {
  ...
  for (let results of shows) {
    ...
    if (
      (genres.length !== 0 && results.show.image && genres.includes(genre)) ||
      (results.show.image && genre === "Select Genre")
    ) {
      flag = true;
